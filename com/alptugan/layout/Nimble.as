	...
	public class Nimble
	{
		...
		private function createNimble( target : DisplayObject,parameterObject : Object ) : void
		{
			...
			switch ( alignment )
			{
				...
				case "TOP":
					parameterObject = {
							x: .5,y: 0,offsetX: -targetWidth >> 1,offsetY: margin
						};
					break;
				
				case "TOP_RIGHT":
					parameterObject = {
							x: 1,y: 0,offsetX: -targetWidth - margin,offsetY: margin
						};
					break;
				
				case "LEFT":
					parameterObject = {
							x: 0,y: .5,offsetX: margin,offsetY: -targetHeight >> 1
						};
					break;
				
				case "MIDDLE":
					parameterObject = {
							x: .5,y: .5,offsetX: -1 - ( targetWidth >> 1 ) - ( margin >> 1 ),offsetY: -1 - ( targetHeight >> 1 ) - ( margin >> 1 )
						};
					break;
				
				case "RIGHT":
					parameterObject = {
							x: 1,y: .5,offsetX: -targetWidth - margin,offsetY: -targetHeight >> 1
						};
					break;
				
				case "BOTTOM_LEFT":
					parameterObject = {
							x: 0,y: 1,offsetX: margin,offsetY: -targetHeight - margin
						};
					break;
				
				case "BOTTOM":
					parameterObject = {
							x: .5,y: 1,offsetX: -targetWidth >> 1,offsetY: -targetHeight - margin
						};
					break;
				
				case "BOTTOM_RIGHT":
					parameterObject = {
							x: 1,y: 1,offsetX: -targetWidth - margin,offsetY: -targetHeight - margin
						};
					break;
			}
		...
		private function checkParameters( parameterObject : Object ) : Object
		{
			...
			if ( parameterObject.x != undefined )
				_newX = ( stageWidth * parameterObject.x ) + parameterObject.offsetX;
			if ( parameterObject.y != undefined )
				_newY = ( stageHeight * parameterObject.y ) + parameterObject.offsetY;
			
			// set default alignment
			if ( parameterObject.alignment == undefined )
				parameterObject.alignment = "MIDDLE";
			
			// set default margin
			if ( parameterObject.margin == undefined )
				parameterObject.margin = 0;
