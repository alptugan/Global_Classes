package  com.alptugan.experiments.qrcodereader {
	...
	public class QRcodereader extends Sprite {
		...
		private var runTime:Timer = new Timer(50);
		...
		private function startReader():void {
			...
			runTime.start();
			runTime.addEventListener("timer", processQR, false, 0, true);
		...
		private function onQrDecodeComplete(e: QRdecoderEvent):void {
			...
			if ( textArray[0] == textArray[1] && textArray[1] == textArray[2] ) {
				...
				}else if(tempString.match("https://")){
					trace("found https");
					foundHTTP = true;
					resultView.text_txt.htmlText = "<a href='"+textArray[0]+"'>"+textArray[0]+"</a>";
					if(autoURL == true){
				...
				//removeEventListener(Event.ENTER_FRAME, processQR);
				runTime.stop();
				if(autoURL == true && foundHTTP == true){
