package com.alptugan.ui
{
	...
	public class AScrollBarJPG extends Root
	{
		[Embed(source="pot.jpg")]
		...
		public function AScrollBarJPG(content:DisplayObjectContainer, w:int,h:int,scWidth:int,scColor:uint,scBgColor:uint,roundness:Array = null,bolFilters:Boolean = true,JPG:Boolean=false )
		{
		...
		private function adjustSize():void
		{
			...
			content.y = Math.round(-((cH - h) * (sc.y / scrollValue)) + defaultPositionY);
		...
		private function createScroller():void
		{
			if(!JPG)
			{
				...
				sc.x =defaultPositionX+ scBg.x - (sc.width - scWidth) * 0.5;
			}
